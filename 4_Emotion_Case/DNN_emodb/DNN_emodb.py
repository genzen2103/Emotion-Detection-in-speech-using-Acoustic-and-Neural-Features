...
if __name__=="__main__":
	emotions={'anger_female':0,'anger_male':1,'boredom_female':2,'boredom_male':3,'happiness_female':4,'happiness_male':5,'neutral_female':6,'neutral_male':7,'sadness_female':8,'sadness_male':9}
	n_hidden1=30
	n_hidden2=15
	n_input=52
	b_size=50
	...
	for emotion in emotions:
		all_emotion_data=np.genfromtxt('emodb_data/train/'+emotion+"_all_features", delimiter=',')
	...
	for cv in xrange(n_cross_val):
		...
		for emotion in emotions:
			for testcasefile in glob.glob('emodb_data/test/'+emotion+'*'):
				total_tests+=1
