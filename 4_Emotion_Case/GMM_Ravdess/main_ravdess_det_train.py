...
def extract_prosody(file_name,emo_folder):
	features=[]
	for line in open("Features_ravdess/train_wavdata/prosody"+"/"+emo_folder+'/'+file_name+"_prosody"):
...
def extract_lpcc(file_name,emo_folder):
	features=[]
	for line in open("Features_ravdess/train_wavdata/LPCC"+"/"+emo_folder+'/'+file_name+"_lpcc"):
		row=map(float,line.strip().split(' ')) 
		row=row[1:]
		features.append(row)
	return np.array(features)
...
if __name__=="__main__":
	...
	all_emotions=sorted(glob.glob('../Ravdess_Dataset/train_wavdata/*'))
	all_emotions=[emo for emo in all_emotions if 'surprised' not in emo and'disgust' not in emo and'fearful' not in emo and 'calm' not in emo ]
	# print all_emotions
	...
	for emotion in all_emotions:
		...
		emotion_name=emotion.replace('../Ravdess_Dataset/train_wavdata/','')
		# print emotion_name
		...
		for sample_file in file_list:
			...
			if Fs==all_emotion_Fs:
				...
				actual_file_name = sample_file.replace('../Ravdess_Dataset/train_wavdata/'+emotion_name+'/','')
