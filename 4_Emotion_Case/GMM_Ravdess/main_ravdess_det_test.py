...
def extract_prosody(file_name,emo_folder):
	features=[]
	for line in open("Features_ravdess/test_wavdata/prosody"+"/"+emo_folder+'/'+file_name+"_prosody"):
...
def extract_lpcc(file_name,emo_folder):
	features=[]
	for line in open("Features_ravdess/test_wavdata/LPCC"+"/"+emo_folder+'/'+file_name+"_lpcc"):
		row=map(float,line.strip().split(' ')) 
		row=row[1:]
		features.append(row)
	return np.array(features)
...
if __name__=="__main__":
	...
	all_emotions=sorted(glob.glob('../Ravdess_Dataset/test_wavdata/*'))
	...
	tct={}
	for e in emotions:
		num_test_cases[e.replace('../Ravdess_Dataset/test_wavdata/','')]=len(os.listdir(e))-1
		tct[e.replace('../Ravdess_Dataset/test_wavdata/','')]=0
